...
const router = Router();
const carts = new ProductManager(__dirname + "/car.json");
const pathJSON = __dirname + "/car.json";
...
router.post("/:cid/product/:pid", async (req, res) => {
  try {
    ...
    if (car) {
      ...
      if (prod) {
        const prodDuplicado = products.findIndex(
        ...
        if (prodDuplicado === -1) {
          ...
          //   console.log("prueba");
          res.status(200).json({ carts: "Producto agregado con exito" });
        } else {
